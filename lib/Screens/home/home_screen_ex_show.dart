...
class _HomeScreenExShowState extends State<HomeScreenExShow> {
  ...
  void _refreshPayloadIfNeeded() {
    ...
    // KEY FIX: Always use currentWeekAuto (auto-calculated) for home screen
    // so it never changes when user taps week tabs in workout screen.
    final autoWeek = workoutProvider.currentWeekAuto;
    final nextKey =
        '$uid|$autoWeek|$splitFingerprint|${DateTime.now().weekday}';
  ...
  @override
  Widget build(BuildContext context) {
    // KEY FIX: Only rebuild when split changes or weekday changes.
    // Do NOT use selectedWeek — use currentWeekAuto so workout tab
    // week selection never affects home screen.
    final splitSignature = context.select<WorkoutProvider, String>((provider) {
      final splitFingerprint = provider.activeSplitDays
          .map((day) => day.trim().toLowerCase())
          .join('|');
      // Use currentWeekAuto, not selectedWeek
      return '${provider.currentWeekAuto}|$splitFingerprint';
    });

    final uid = FirebaseAuth.instance.currentUser?.uid ?? '';
    final autoWeek = context.read<WorkoutProvider>().currentWeekAuto;
    final nextKey = '$uid|$autoWeek|$splitSignature|${DateTime.now().weekday}';
  ...
  Future<_ProgressionPayload> _buildPayload() async {
    ...
    // KEY FIX: Always use currentWeekAuto for home screen — never selectedWeek
    final currentWeekNumber = workoutProvider.currentWeekAuto;
    final currentDayId = todayIndex + 1;
    ...
    try {
      ...
      // KEY FIX: Compute previous week correctly with 4-week loop
      // Current week 1 → look at week 4 (previous loop)
      // Current week 2 → look at week 1
      // Current week 3 → look at week 2
      // Current week 4 → look at week 3
      final historyWeekNumber = currentWeekNumber == 1
          ? 4
          : currentWeekNumber - 1;

      final historyExercises = await _loadPreviousWeekDayExercises(
        uid: uid,
        historyWeekNumber: historyWeekNumber,
        currentDayId: currentDayId,
      );

      // If no history at all — show simple empty state (not dummy data)
      if (historyExercises.isEmpty) {
        if (templateExercises.isEmpty) {
          // User has no exercises set up at all — simple empty state
          final dayNames = [
            'Monday',
            'Tuesday',
            'Wednesday',
        ...
        // User has template exercises but no logged history yet
        // Show template targets without dummy weights
        final parsedTemplate = templateExercises
            .map((raw) => _ExerciseSnapshot.fromMap(raw))
            .where((e) => e.name.trim().isNotEmpty)
            .toList(growable: false);
      ...
      // Has history — show progression targets
      final effectiveTemplate = templateExercises.isNotEmpty
          ? templateExercises
          : historyExercises
                .whereType<Map>()
                .map((e) => Map<String, dynamic>.from(e))
      ...
      final hydrated = _hydrateExercisesWithHistory(
        templateExercises: effectiveTemplate,
        historyExercises: historyExercises,
      );
  ...
  // KEY FIX: renamed param to historyWeekNumber for clarity
  Future<List<dynamic>> _loadPreviousWeekDayExercises({
    required String uid,
    required int historyWeekNumber,
    required int currentDayId,
  }) async {
    final daySnap = await FirebaseFirestore.instance
        .collection('users')
...
// ─────────────────────────────────────────────────────────────────────────────
// UI WIDGETS (unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          const Text(
            'Log your workout today to unlock\nprogressive overload tracking!',
            textAlign: TextAlign.center,
            style: TextStyle(
              color: Color(0xFF8B8B8B),
              fontSize: 14,
