...
class StepService extends ChangeNotifier {
  ...
  static const String _lastResetDateKey =
      'last_reset_date'; // Stores last reset date.
  ...
  Stream<int> get todayStepsStream =>
      _stepsController.stream; // Expose today steps stream.
  ...
  Future<void> initializeStepService() async {
    ...
    _lastResetDate =
        preferences.getString(_lastResetDateKey) ?? ''; // Load last reset date.
  ...
  void onStepCount(StepCount event) {
    ...
    final incomingSteps = event.steps < 0
        ? 0
        : event.steps; // Normalize hardware steps.
    ...
    int calculatedSteps =
        incomingSteps - _baselineSteps; // Compute daily steps.
    if (calculatedSteps < 0) {
      // Guard negative totals.
  ...
  /// Emits today's steps to listeners, skipping duplicate values.
  void _emitSteps(int steps) {
    // Emit today steps to stream.
    if (steps == _lastEmittedSteps) {
